...
export const aiTrade = (hand) =>{
    ...
    if( hearts.length<3 && hearts.length>0 && tradedCards <3){
        for(let j=0; j<hearts.length; j++){
            tradedCards.push(hearts[j])
        }
        hearts = []
    }
    if(spades.length<3 && spades.length>0 && tradedCards <2){
        for(let k=0; k<spades.length; k++){
            tradedCards.push(spades[k])
        }
        spades = []
    }
    if(diamonds.length<3 && diamonds.length>0 && tradedCards<3){
        for(let l=0; l<diamonds.length; l++){
            tradedCards.push(diamonds[l])
        }
        diamonds = []
    }
    if(clubs.length<3 && clubs.length>0 && tradedCards<3){
